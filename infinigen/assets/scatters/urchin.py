...
def apply(obj, n=5, selection=None, density=U(0.2, 0.8)):
    ...
    factories = list(UrchinFactory(np.random.randint(1e5), extrude_height=("clip_gaussian", U(1.5, 2.5), 0.7, 0.5, 5.0)) for _ in range(n_species))
    # Add baby urchin
    factories.append(UrchinFactory(np.random.randint(1e5), min_spike_scale=0.2,
                                   spike_prob=.6, extrude_height=("clip_gaussian", U(1.0, 2.0), 1.2, 0.1, 3.5)))
