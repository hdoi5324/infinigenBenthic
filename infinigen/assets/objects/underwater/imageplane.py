...
class ImagePlaneFactory(AssetFactory):

    def __init__(self, factory_seed, data_path="../../BenthicSynData/outputs",
    ...
    def create_asset(self, **kwargs) -> bpy.types.Object:
        # obj = new_cube()
        # obj.name = "imageplane"
        image_pixels, mask_pixels, width, height, polygon = self.get_coco_image_and_mask()
        edges = []
        ...
        kwargs = {'image_pixels': image_pixels,
                  ...
                  'width': width,
                  'height': height}
        # material = surface.add_material(obj, shader_image_plane, selection=None, input_kwargs=kwargs)
    ...
    def get_coco_image_and_mask(self):
        ...
        return get_random_mask(image, target)


def shader_image_plane(nw: NodeWrangler, image_pixels, mask_pixels, width, height):
    # Code generated using version 2.6.5 of the node_transpiler

    random_id = random.randint(0, 5000)
    image_name = f"coco_crop_image{random_id}.png"
    mask_name = f"coco_crop_mask{random_id}.png"
    img = bpy.data.images.new(name=image_name, width=width, height=height)
    img.pixels = image_pixels
    # write image
    # img.filepath_raw = image_name
    # img.file_format = 'PNG'
    # img.save()

    mask = bpy.data.images.new(name=mask_name, width=width, height=height)
    mask.pixels = mask_pixels
    # mask.filepath_raw = mask_name
    # mask.file_format = 'PNG'
    # mask.save()

    # Use existing images
    # image_filename = "maskedv2.jpg"
    # image_mask = "mask.jpg"
    # image_dir = "./"
    # bpy.ops.image.open(filepath=image_name, directory=image_dir, relative_path=False, files=[{"name":image_name}])
    # bpy.ops.image.open(filepath=mask_name, directory=image_dir, relative_path=False, files=[{"name":mask_name}])
    ...
    color_ramp.color_ramp.elements[0].position = 0.4114
    color_ramp.color_ramp.elements[0].color = [0.0000, 0.0000, 0.0000, 1.0000]
    color_ramp.color_ramp.elements[1].position = 0.4250
    color_ramp.color_ramp.elements[1].color = [0.9630, 0.9630, 0.9630, 1.0000]

    rgb_to_bw = nw.new_node('ShaderNodeRGBToBW', input_kwargs={'Color': color_ramp.outputs["Color"]})
    #rgb_to_bw = nw.new_node('ShaderNodeRGBToBW', input_kwargs={'Color': image_texture_1.outputs["Color"]})

    principled_bsdf = nw.new_node(Nodes.PrincipledBSDF,
                                  input_kwargs={'Base Color': image_texture.outputs["Color"],
                                                'Alpha': rgb_to_bw})
...
def get_random_mask(image, targets, use_bbox=False, pixel_size_in_m=0.01):
    ...
    # Image.fromarray(np.array(image, dtype=np.uint8)).save(f"{id}image.jpg")
    # Image.fromarray((mask * 255).astype(np.uint8)).save(f"{id}mask.jpg")
    ...
    return image_pixels, mask_pixels, width, height, polygon
...
def geometry_nodes(nw: NodeWrangler, image_pixels, mask_pixels, width, height):
    # Code generated using version 2.6.5 of the node_transpiler
    # Code generated using version 2.6.5 of the node_transpiler
    ...
    # group_output = nw.new_node(Nodes.GroupOutput, input_kwargs={'Geometry': fill_curve}, attrs={'is_active_output': True})
    # quadrilateral = nw.new_node('GeometryNodeCurvePrimitiveQuadrilateral')
    # quadrilateral.inputs[0].default_value = .2
    # quadrilateral.inputs[1].default_value = .2

    # fill_curve = nw.new_node(Nodes.FillCurve, input_kwargs={'Curve': quadrilateral})

    # group_input = nw.new_node(Nodes.GroupInput,
    #                          expose_input=[('NodeSocketGeometry', 'Geometry', None)])
    # ('NodeSocketMaterial', 'Material', surface.shaderfunc_to_material(shader_image_plane))])        
    # transform_geometry = nw.new_node(Nodes.Transform, input_kwargs={'Geometry': fill_curve, 'Rotation': (0.0000, 0.0000, -1.5708)})

    set_material = nw.new_node(Nodes.SetMaterial, input_kwargs={'Geometry': fill_curve,
    ...
    group_output = nw.new_node(Nodes.GroupOutput,
                               input_kwargs={'Geometry': set_material, 'UV': uv_unwrap},
                               attrs={'is_active_output': True})
...
def place_colourboard(cam, terrain_bvh, n, alt, dist_range):
    ...
    for p in points:
        p[-1] += alt_offset
        obj = ImagePlaneFactory(1).create_asset()
        obj.location = p
        alt_offset += alt
